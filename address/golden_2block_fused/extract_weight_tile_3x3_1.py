...
def main():
    ...
    input_file = "../Fused-Block-CNN/address/golden_2block_fused/hex/weight_1.hex"
    tile = args.filter_count // args.pe
    weight_size = 3 * 3 * args.weight_channel
    offset = weight_size * (args.pe - 1)
    num_segments = args.filter_count // args.pe

    for pe in range(args.pe):
        output_file = f"../Fused-Block-CNN/address/golden_2block_fused/hex/weight_1_PE{pe}.hex"
        read_and_write_file(input_file, output_file, weight_size, offset, num_segments, pe, 3, args.weight_channel)
