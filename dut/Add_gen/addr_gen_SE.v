...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if ( (row_index_KERNEL == (KERNEL_W  *num_of_tiles) -4 )  ) begin 
            ...
            predict_line_addr_fetch_ifm         <= 0;// predict_line_addr_fetch_ifm + (IFM_W *IFM_C ) ;
            else 
            predict_line_addr_fetch_ifm         <= 0;// window_start_addr_ifm  ;
        end
        ...
        if ( (row_index_KERNEL == (KERNEL_W  * num_of_tiles) -3 ) && ( col_index_KERNEL == KERNEL_W -1  )  && ( tiles_count == num_of_tiles_for_PE -1  ) ) begin 
            predict_window_addr_fetch_ifm       <= 0;// window_start_addr_ifm + ( IFM_C << stride_shift );
            predict_line_addr_fetch_ifm         <= 0;//window_start_addr_ifm + ( IFM_C << stride_shift );
        end
        ...
        if ( (row_index_KERNEL == (KERNEL_W * num_of_tiles) -2 ) && ( col_index_KERNEL == KERNEL_W -1  ) && ( tiles_count == num_of_tiles_for_PE -1  ) && ( row_index_OFM == OFM_W -1 ) ) begin 
            ...
            predict_window_OFM_addr_fetch_ifm   <= 0;// window_start_addr_ifm + ( KERNEL_W *IFM_C ) + stride_offset_for_col + skip_a_pixel;
            predict_window_addr_fetch_ifm       <= 0;//window_start_addr_ifm + ( KERNEL_W *IFM_C ) + stride_offset_for_col + skip_a_pixel;
            predict_line_addr_fetch_ifm         <= 0;//window_start_addr_ifm + ( KERNEL_W *IFM_C ) + stride_offset_for_col + skip_a_pixel;
        end
