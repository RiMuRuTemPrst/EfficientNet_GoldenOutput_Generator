...
reg [15:0] count_for_a_Window_of_a_tile;
...
reg [31:0] predict_next_tiles_addr_fetch_ifm;
...
always @(*) begin  
    ...
    'h2: // stride_offset_for_col  =   ( IFM_W << (stride_shift-1) << IFM_C_shift );
            stride_offset_for_col  =    IFM_W  * IFM_C ;
    endcase
end
reg [31:0] stride_offset_for_row;
always @(*) begin  
    case ( stride )
    'h1:    stride_offset_for_row  =   0;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        predict_next_tiles_addr_fetch_ifm   <= 'h0;
        ...
        if ( (row_index_KERNEL == (KERNEL_W) -2 )  ) begin 
            if ( col_index_KERNEL != (KERNEL_W -1)) begin
            predict_line_addr_fetch_ifm         <= predict_line_addr_fetch_ifm + (IFM_W *IFM_C ) ;
            end else begin
            predict_next_tiles_addr_fetch_ifm   <= window_start_addr_ifm + num_of_PE ;
            predict_line_addr_fetch_ifm         <= window_start_addr_ifm + num_of_PE ;
            end
        end


        if ( (row_index_KERNEL == (KERNEL_W ) -2 ) && ( col_index_KERNEL == KERNEL_W -1  )  && ( tiles_count == num_of_tiles_for_PE -1  ) ) begin 
            predict_window_addr_fetch_ifm       <= window_start_addr_ifm +num_of_PE + stride_offset_for_row;
            predict_next_tiles_addr_fetch_ifm   <= window_start_addr_ifm +num_of_PE + stride_offset_for_row;
            predict_line_addr_fetch_ifm         <= window_start_addr_ifm +num_of_PE + stride_offset_for_row;
        end
        ...
        if ( row_index_KERNEL == (KERNEL_W  -2) && ( col_index_KERNEL == KERNEL_W -1  ) && ( tiles_count == num_of_tiles_for_PE -1  ) && ( row_index_OFM == OFM_W -1 ) ) begin 
            predict_window_OFM_addr_fetch_ifm   <= window_start_addr_ifm +num_of_PE+ ( (KERNEL_W-1) *IFM_C ) + stride_offset_for_col + skip_a_pixel;
            predict_next_tiles_addr_fetch_ifm   <= window_start_addr_ifm +num_of_PE+ ( (KERNEL_W-1) *IFM_C ) + stride_offset_for_col + skip_a_pixel;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        case (current_state_IFM)
            ...
            FETCH_WINDOW: begin
                count_for_a_Window_of_a_tile  <= count_for_a_Window_of_a_tile + 'b1;
                if(count_for_a_Window_of_a_tile <num_of_tiles_for_PE*num_of_KERNEL_points -1 ) begin
                    if ( tiles_count < num_of_tiles_for_PE ) begin
                        count_for_a_Window  <= count_for_a_Window + 'b1;
                        if ( count_for_a_Window < num_of_KERNEL_points  -1) begin
                        //if ( count_for_a_Window < (num_of_KERNEL_points <<  (IFM_C_shift - num_of_mul_in_PE_shift + OFM_C_shift - total_PE_shift))   -1) begin
                            ...
                            if (row_index_KERNEL    <   (KERNEL_W ) -1 ) begin 
                                row_index_KERNEL    <= row_index_KERNEL + 1'b1;
                                addr_fetch_ifm      <= addr_fetch_ifm + IFM_C;               
                            end  
                            else
                            begin
                                ...
                                begin
                                    col_index_KERNEL    <= 'b0;
                                    //if ( tiles_count    <   num_of_tiles_for_PE)  
                                    begin  
                            ...
                            addr_fetch_ifm          <= predict_next_tiles_addr_fetch_ifm ;
                            count_for_a_Window      <= 'b0;
                            row_index_KERNEL        <= 'b0;
                            col_index_KERNEL        <= 'b0;
                            tiles_count             <=  tiles_count +1'b1 ;
                            window_start_addr_ifm   <= predict_next_tiles_addr_fetch_ifm;
                        end
                    end else begin
                    ...
                    count_for_a_Window_of_a_tile<=0;
                    count_for_a_Window      <= 'b0;
                    row_index_KERNEL        <= 'b0;
                    col_index_KERNEL        <= 'b0;
                    tiles_count <=0;
            ...
            NEXT_WINDOW: begin
                ...
                if (count_for_a_Window_of_a_tile == 0)    count_for_a_Window_of_a_tile  <= count_for_a_Window_of_a_tile +1;    
...
always @(*) begin
    case (current_state_FILTER)
        START_ADDR_FILTER: begin
            ...
            // end
            if (addr_valid_ifm ==1 ) next_state_FILTER   =  FETCH_FILTER ;
            else next_state_FILTER   =  START_ADDR_FILTER ;
        end
        FETCH_FILTER: begin
    ...
    if (!rst_n) begin
        ...
        case (current_state_FILTER)
            START_ADDR_FILTER: begin
                ...
                if ( addr_valid_ifm ==1 )  addr_fetch_filter    <=  addr_fetch_filter + 'h1; 
                else                       addr_fetch_filter    <=  addr_in;
                //end 
            end
            ...
            FETCH_FILTER: begin
                if ( addr_valid_ifm ==1 ) begin
                    ...
                    addr_fetch_filter           <=  addr_fetch_filter + 'h1;  
                end                 
            end
            default: begin
...
assign req_addr_out_filter = count_for_a_Window_of_a_tile;
