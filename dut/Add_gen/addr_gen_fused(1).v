module  addr_gen_fused #(
    parameter TOTAL_PE   = 16,
    parameter DATA_WIDTH = 32,
    parameter total_PE_shift =4,
    parameter num_of_mul_in_PE =16
)(
    input  wire clk,
    input  wire rst_n,
...
reg [15:0] count_for_a_OFM_in_a_pipeline_tile;
...
always @(*) begin
    ...
    case (current_state_IFM)
        START_ADDR_IFM: begin
            if ( ready && (count_for_a_OFM < OFM_W*OFM_W -1 )  ) begin
        ...
        NEXT_WINDOW: begin
            ...
            if (count_for_a_OFM_in_a_pipeline_tile < OFM_W*num_of_line_for_pipeline -1 )   begin
                next_state_IFM  =   FETCH_WINDOW ;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        //     if (count_for_a_OFM_in_a_pipeline_tile =='h0 && count_for_a_Window=='h0 && tiles_count=='h0)
        //         finish_for_PE <='h0;
        //     else begin
        //         finish_for_PE<= 'b1;
        //     end
        ...
        if ( ( row_index_KERNEL == 'h0 )&& ( col_index_KERNEL ==  'h0 )  ) begin 
            if ( current_state_IFM != PENDING) begin
               if ( current_state_IFM != START_ADDR_IFM) begin
                done_window <= 'b1;
               end
               else begin
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        count_for_a_OFM_in_a_pipeline_tile <=0;
        ...
        case (current_state_IFM)
            START_ADDR_IFM: begin
                if ( count_for_a_OFM_in_a_pipeline_tile == 0) begin 
            ...
            NEXT_WINDOW: begin
                ...
                count_for_a_OFM_in_a_pipeline_tile <= count_for_a_OFM_in_a_pipeline_tile+1;
                tiles_count                 <= 0;
                ...
                if (count_for_a_OFM_in_a_pipeline_tile    <   OFM_W*num_of_line_for_pipeline-1) begin
                    ...
                    begin
                     ...
                     if ( count_for_a_OFM_in_a_pipeline_tile >= OFM_W*num_of_line_for_pipeline-1 ) done_compute    <=  1;
                        else done_compute    <=  0;
                end
                if (count_for_a_OFM >= OFM_W*OFM_W -1 )   begin
                    done_compute_all <=1;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        case (current_state_FILTER)
            START_ADDR_FILTER: begin
                if ( ready ) begin
                    ...
                    if ( addr_valid_ifm ==1 ) begin
                        if (next_state_IFM == START_ADDR_IFM) begin
                            addr_fetch_filter <= addr_in;
                        end else begin
                         addr_fetch_filter    <=addr_fetch_filter + num_of_mul_in_PE; 
                        end
                    end
                end 
                else begin
            ...
            FETCH_FILTER: begin
                if ( next_state_FILTER ==  START_ADDR_FILTER)  addr_fetch_filter    <=  addr_in ;//addr_fetch_filter + num_of_mul_in_PE; 
                if ( addr_valid_ifm ==1 ) begin
                    if (count_for_a_Window ==  (num_of_KERNEL_points * num_of_tiles * num_of_tiles_for_PE  -1)) begin
                    //if ( count_for_a_Window == (num_of_KERNEL_points <<  (IFM_C_shift - num_of_mul_in_PE_shift + OFM_C_shift - total_PE_shift)) -1 ) begin
                    addr_fetch_filter           <=  addr_in;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        case (current_state_FILTER)
            START_ADDR_FILTER: begin
                if ( ready ) begin
                    if ( addr_valid_ifm ==1 )  coef_for_multiply_addr    <=  coef_for_multiply_addr + num_of_mul_in_PE; 
                end 
                else begin
            ...
            FETCH_FILTER: begin
                if ( addr_valid_ifm ==1 ) begin
                    ...
                    coef_for_multiply_addr           <= coef_for_multiply_addr + num_of_mul_in_PE;  
                end                 
            end
            default: begin
